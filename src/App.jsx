...
export default function App() {
  ...
  const [feedback, setFeedback] = useState(
    "Press Start Listening, then play the note."
  );
  ...
  function nextScaleStep() {
    ...
    setValveInput("");
    setFeedback(
      `Scale: ${selectedTonic} major — degree ${idx + 1}/${seq.length}`
    );
  }
  ...
  function loop() {
    ...
    if (freq > 0 && freq < 1200) {
      ...
      const playedName = nameFromMidi(midi);
      const targetName = parseWritten(currentNote).name;
      setLiveCents(cents);
      setLivePlayed(playedName);
      setLiveOK(Math.abs(cents) <= 25 && playedName === targetName);
    }
  ...
  function submitAttempt() {
    ...
    setBothOK((b) => b + (bothGood ? 1 : 0));
    setStreak((s) => (bothGood ? s + 1 : 0));
    setHistory((h) =>
      [
        {
          ts: Date.now(),
    ...
    setFeedback(
      bothGood
        ...
        : `Keep refining: ${targetName} (${liveCents}¢) — valves ${
            valvesGood ? "OK" : "check"
          }`
    );
  ...
  useEffect(() => {
    if (mode === "flashcards") nextFlashcard();
      ...
      setFeedback(
        `Scale: ${selectedTonic} major — degree 1/${currentScale.length}`
      );
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button onClick={startListening} style={btnPrimary}>
          {listening ? "Listening…" : "Start Listening"}
        </button>
        {listening && (
          <button onClick={stopListening} style={btn}>
            Stop
          </button>
        )}
        <button onClick={resetSession} style={btn}>
          Reset Session
        </button>
      </div>
      ...
      >
        ...
        <Badge
          label={`${liveCents > 0 ? "+" : ""}${liveCents}¢`}
          ok={Math.abs(liveCents) <= 25}
        />
      </div>
      ...
      >
        {mode === "flashcards" ? (
          <button onClick={nextFlashcard} style={btnSuccess}>
            Next Note
          </button>
        ) : (
          <button onClick={nextScaleStep} style={btnSuccess}>
            Next Degree
          </button>
        )}
        <button onClick={submitAttempt} style={btnPurple}>
          Submit
        </button>
      </div>
      ...
      >
        ...
        >
          ...
          <tbody>
            ...
            {history.length === 0 && (
              <tr>
                <td style={{ padding: 8, textAlign: "center" }} colSpan={5}>
                  No attempts yet.
                </td>
              </tr>
            )}
      ...
      <p style={{ fontSize: 12, color: "#555", marginTop: 8 }}>
        Tip: Open preview in a new window (↗︎) so the browser shows the mic
        prompt.
      </p>
    </div>
  );
...
function Stat({ label, value }) {
  return (
    ...
    >
      <div
        style={{ fontSize: 11, textTransform: "uppercase", color: "#6b7280" }}
      >
        {label}
      </div>
